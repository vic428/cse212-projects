...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If we're removing the head, call RemoveHead
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If we're removing the tail, call RemoveTail
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                // For any other location, need to reconnect the links
                else
                {
                    curr.Prev!.Next = curr.Next; // Connect previous node to the node after current
                    curr.Next!.Prev = curr.Prev; // Connect node after current to previous node
                }
    ...
    {
        // Search through the entire list for all occurrences of 'oldValue'
        Node? curr = _head;
        while (curr is not null)
        {
